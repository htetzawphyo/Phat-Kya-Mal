    ...
    <div class="p-2 sm:ml-64" >
        <div class="p-4 mt-12">
            <div class="flex justify-start items-center">
                <h3 class="text-gray-900 font-semibold text-2xl">Book Lists</h3>
            </div>
            ...
            <div class="relative overflow-x-auto shadow-lg rounded-md">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody class="">
                        <tr v-if="getBooks == 0" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900">
                            <th colspan="5" scope="row" class="w-1 text-center  py-4 font-medium text-gray-500 whitespace-nowrap dark:text-gray-500">
                                No Data
                            </th>
                        </tr>
                        <tr v-for="(book,  index) in getBooks" :key="index" class="text-xs bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900" >
                            ...
                            <td class="px-6 py-4 flex">
                                ...
                                <font-awesome-icon icon="fa-solid fa-trash" @click="deleteBook(book.id)"
...
<script setup>
    ...
    const { getBooks, getMeta, getIndex, getDetailBook } = storeToRefs(bookStore);
    bookStore.getBook();

    const searchBox = ref('');    
    
    const handlePageUpdate = (currentPage) => {
        bookStore.getBook(searchBox.value, currentPage)
    }
    
    const search = debounce.debounce(e => {
        const value = e.target.value;
        searchBox.value = value;
        bookStore.getBook(value);
    })

    const deleteBook = (id) => {
        confirm('Are you sure you want to Delete?') ? bookStore.deleteBook(id) : '';
    }
